...
Abbildung \label{fig:tcsiteTCserverCommunication} veranschaulicht die Kommunikation zwischen TCsite und TCserver während der Konfiguration. Das Configuration-Modul führt wiederholt zustandslose Aufrufe (configure) der Konfigurationsengine durch, welche eine Antwort produziert (result). Dabei weichen configure und result von den Definitionen der Konfigurationsaufgabe und -lösung aus Kapitel \ref{subsubsection:begriffsuberblick} ab \citep{tactonTCsiteDevelopmentManual}. Laut Kapitel \ref{subsubsection:Konfigurationslösung} müssen interaktive Konfiguratoren einen Mechanismus zum Merken des Konfigurationszustands besitzen. Eben wurde bemerkt, dass die Aufrufe zustandslos sind. Also wird kein serverseitiger Kontext aufgebaut, der die bisherigen Entscheidungen enthält. Die Alternative lautet, dass jeder Aufruf der Konfigurationsengine nicht nur das Konfigurationsmodell und die gewählte Option des Anwenders, sondern auch den Konfigurationszustand enthält \citep{tactonTCsiteApiDocu}. Die Aufrufe werden ab jetzt als Konfigurationsengine-Input (KE-Input) bezeichnet. Die Antwort liefert neben der Konfigurationslösung auch die Datengrundlage zum Rendern der Konfigurationsoberfläche \citep{tactonTCsiteApiDocu}. Dementsprechend wir Verarbeitungsergebnis des KE-Input ab jetzt als Konfigurationsengine-Output (KE-Output) bezeichnet.

Die Kommunikation läuft in drei Phasen ab. Der erste KE-Input beinhaltet noch keine Anwenderwahl. Sie dient dem initialen Aufbau der Konfigurationsoberfläche entsprechend der Execution, die im Konfigurationsmodell definiert wurde. Die Konfiguration startet immer in Step 1. Die Steps müssen in der vordefinierten Reihenfolge abgearbeitet werden. Nun beginnt die Loop-Phase. Nach jeder Wahl einer Option wird die Konfigurationsengine aufgerufen und daraufhin die Oberfläche neu gerendert. Die Oberfläche spiegelt dabei den Konfigurationszustand wieder, d.h. die bisher gewählten Optionen und Eingaben werden hervorgehoben. Wird die Konfiguration im letzten Step vom Anwender abgeschlossen, erfolgt eine letzter Aufruf der Konfigurationsengine. Aus dem KE-Output ist die Konfigurationslösung, d.h. die finale Instanziierung, extrahierbar.
...
\subsubsection{Fazit}
Der Produktkatalog von TCsite besteht nicht aus explizit ausdefinierten Varianten, sondern Produkten mit Konfigurationsmodellen. Erst im Rahmen von Quotations werden aus den Konfigurationsmodellen kundenspezifische Varianten generiert. Dies passiert im Configuration-Modul. Es führt die Konfiguration durch, indem sie KE-Inputs definiert, die Konfigurationsengine aufruft und aus den KE-Outputs die Konfigurationsoberfläche rendert.

Grundlegende Idee ist, durch Auslesen des KE-Outputs eine Oberfläche in einem externen System aufzubauen. Über diese Oberfläche kann der Anwender Optionen wählen, die dann an das Configuration-Modul weitergeleitet werden. Dort wird in Verbindung mit dem Konfigurationsmodell und dem Konfigurationszustand ein KE-Input definiert. Über den KE-Output kann der Konfigurationsoberfläche im externen System wiederum aktualisiert werden. Die für diese Variante relevanten Informationen müssen nur irgendwie zwischen dem externen System und dem Configuration-Modul transportiert werden. Das Pluginkonzept erlaubt die Definition von Serviceendpunkten über Webcontroller. An diesen können HTTP-Requests ausgewertet und beantwortet werden. Dadurch kann ein Webservice implementiert werden, der eine Schnittstelle für externe Konfigurationsoberflächen bietet.
...
\subsection{Fazit}
Shopware bietet in der Standardinstallation keine Funktionalität, die der Definition eines Konfigurators gerecht wird. Stattdessen wird eine Variantenselektion unterstützt. Dadurch werden die in Kapitel \ref{subssubsection:Produktklassifizierung}
 beschriebenen \ac{ATO}-Produkte abgebildet, da alle Varianten eines Artikels vordefiniert sind. Die Analyse des Tacton-Konfigurationsmodells in Kapitel \ref{subsection:Konfigurationsmodell} hat ergeben, dass es auch \ac{MTO}-Produkte abbilden kann. Eine Integration des Tacton-Produktkonfigurators in Shopware würde somit dessen Angebotsspektrum in Bezug auf Produktionskonzepte erweitern.
 
In Kapitel \ref{subsubsection:tcsiteArchitektur} wurde festgestellt, dass TCsite die Konfiguration über das Configuration-Modul realisiert.  Die Analyse der Erweiterbarkeit in Kapitel \ref{subsubsection:Erweiterbarkeit} hat ergeben, dass die Kommunikation mit externen Systemen über Webcontroller vorgesehen ist. Deren Implementation geschieht in Form eines TCsite-Plugins. Für die Arbeit bedeutet dies, dass ein Plugin zu erstellen ist, welches eine Konfigurationsschnittstelle für externe Systeme anbietet. 

In Kapitel \ref{subsubsection:shopwareErweiterbarkeit} wurde besprochen, dass...
