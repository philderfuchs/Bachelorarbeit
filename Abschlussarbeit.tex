...
Die Einheit aus Konfigurationsmodell und den Kundenanforderungen wird als \textbf{Konfigurationsaufgabe} bezeichnet \citep{felferning14}. Auf dessen Grundlage kann die gewünschte Konfiguration errechnet werden. Demzufolge ist der Begriff Konfiguration überladen: er bezeichnet sowohl den technischen Prozess der Lösungsfindung als die Lösung selbst. Im Folgenden werden daher die Begriffe \textbf{Konfigurationsprozess} sowie \textbf{Konfigurationslösung} verwendet. Der Konfigurationsprozess, der zur Konfigurationslösung führt, wird von einem System durchgeführt. Dieses wird als \textbf{Konfigurator} bezeichnet.
...
\item Die \textbf{Auswertungskomponente} präsentiert der Konfiguration in einer Form, welche eine Interpretation der Variante außerhalb des Konfigurators erlaubt. Dies können zum Beispiel Stücklisten, Konstruktionszeichnungen und Arbeitspläne sein.
\end{compactitem}
...
Der Produktkatalog von TCsite besteht nicht aus explizit ausdefinierten Varianten, sondern Produkten mit Konfigurationsmodellen. Erst innerhalb einer Quotation werden aus den Konfigurationsmodellen kundenspezifische Varianten generiert. Dies passiert im Configuration-Modul. Die Konfiguration geschieht durch die Definition von KE-Inputs, die mit KE-Outputs beantwortet werden. Ein KE-Input wird im Configuration-Modul zusammengestellt. Es besteht aus dem Konfigurationsmodell, der gewählten Option und allen vergangenen gewählten Optionen (Konfigurationszustand). Das Configuration-Modul nimmt den KE-Output entgegen und rendert die Konfigurationsoberfläche auf dessen Grundlage neu.

Die Daten aus dem KE-Output, die zum Rendern der Konfigurationsoberfläche genutzt werden, können auch einem externen System zur Verfügung gestellt werden, um dort eine entsprechende Oberfläche zu erstellen. Über diese kann der Anwender Optionen wählen, die dann an das Configuration-Modul zurückgegeben werden. Dort wird in Verbindung mit dem Konfigurationsmodell und dem Konfigurationszustand ein KE-Input definiert. Über den KE-Output kann die Konfigurationsoberfläche im externen System wiederum aktualisiert werden. Die für diesen Vorgang relevanten Informationen müssen nur irgendwie zwischen dem externen System und dem Configuration-Modul transportiert werden. Das Pluginkonzept erlaubt die Definition von Serviceendpunkten über Webcontroller. An diesen können HTTP-Requests ausgewertet und beantwortet werden. Dadurch kann ein Webservice implementiert werden, der eine Schnittstelle für externe Konfigurationsoberflächen bereitstellt.
...
Vorausgehend wurde der Lebenszyklus einer Variante in Shopware vorgestellt. Dieser beginnt mit dem Anlegen der Variante im Backend. Das stellt einen Unterschied zu der Umsetzung in TCsite dar, welche in Kapitel \ref{subsubsection:tcsiteArchitektur}
 besprochen wurde. Hier beginnt der Lebenszyklus erst mit der Konfiguration.

\subsection{Fazit}
Shopware bietet in der Standardinstallation keine Funktionalität, die der Definition eines Konfigurators gerecht wird. Stattdessen wird eine Variantenselektion durchgeführt. Da alle Varianten expliziert vordefiniert sind, werden dadurch die in Kapitel \ref{subssubsection:Produktklassifizierung} beschriebenen \ac{ATO}-Produkte abgebildet. Die Analyse in Kapitel \ref{subsection:Konfigurationsmodell} hat ergeben, dass Konfigurationsmodells auch \ac{MTO}-Produkte abbilden kann. Eine Integration des Tacton-Produktkonfigurators in Shopware würde somit dessen Angebotsspektrum in Bezug auf Produktionskonzepte erweitern.
 
Im Fazit der TCsite-Analyse (Kapitel \ref{subsubsection:tcsiteFazit}) wurde
geschlussfolgert, dass eine Konfigurationsoberfläche auch in einem externen System umsetzbar ist. Über Webcontroller kann ein Webservice implementiert werden, mit der die Konfigurationsoberfläche kommuniziert. Webcontroller werden per Plugin definiert. Für die Arbeit bedeutet dies, dass ein Plugin zu erstellen ist, welches eine Konfigurationsschnittstelle für externe Konfigurationsoberflächen anbietet. Das Plugin versorgt das externe System mit den nötigen Informationen zum Rendern der Oberfläche. Im Gegenzug teilt das externe Systeme dem Plugin die gewählten Optionen des Anwenders mit. Das Configuration-Modul kann auf dieser Grundlage einen KE-Input definieren. Die weiteren Bestandteile des KE-Input sind das Konfigurationsmodell, auf welches sich die aktuell gewählte Option bezieht, und der Konfigurationszustand, der die bisher gewählten Optionen speichert. Diese Bestandteile bleiben nach wie vor in der Obhut von TCsite bleiben, damit im externen System kein zusätzlicher Aufwand für die Dateiverwaltung entsteht.

In Kapitel \ref{subsubsection:shopwareErweiterbarkeit} wurde das Plugin-Konzept von Shopware vorgestellt. Die logischen Komponenten, Datenhaltung und Templates sind erweiterbar. Letzteres ermöglicht das Rendern einer Konfigurationsoberfläche in Shopware. Die Analyse der Einkaufsvorgangs mit konfigurierbaren Artikeln in Kapitel \ref{subsubsection:shopwareKonfiguration}, impliziert wo die Konfigurationsoberfläche gerendert werden muss. Außerdem in der Analyse des Einkaufsvorgangs gezeigt, wie  konfigurierte Artikel ihren Lebenszyklus im Warenkorb fortsetzen. Das impliziert Änderungen über die reine Integration einer Konfigurationsoberfläche hinaus. Für die Arbeit bedeutet das, dass ein Shopware-Plugin zu erstellen ist, welches (1) eine Konfigurationsoberfläche bereitstellt und (2) die konfigurierten Artikel in den Einkaufsvorgang integriert.

Nach diesem Grobkonzept der jeweiligen Verantwortungsbereiche wird in der folgenden Anforderungsanalyse definiert, was die beiden Plugins im Details leisten müssen.
\pagebreak
...
\end{enumerate}
Die Funktionalität der Konfigurationsoberfläche kann näher beschreiben werden. Aus der Analyse der Konfiguration in TCsite (siehe Kapitel \ref{subsubsection:tcsiteArchitektur}) ergibt sich die funktionale Anforderung SW.F02:
\begin{enumerate}[SW.F02:]\bfseries
\item Die Konfigurationsoberfläche muss eine Konfiguration äquivalent zu TCsite ermöglichen. 
\end{enumerate}
...
Aus der Analyse des Einkaufsvorgangs in Kapitel \ref{subsubsection:shopwareKonfiguration} geht hervor, dass der Lebenzyklus einer Variante durch die Konfiguration gerade erst beginnt. Weitere Stationen sind der Warenkorb und Checkout. Daraus resultiert Anforderung SW.03, welche die Basis der folgenden Anforderungen darstellt:
\begin{enumerate}[SW.F03:]\bfseries
\item Die aus der Konfiguration resultierende Variante muss sich in den Einkaufsvorgang integrieren.
\end{enumerate}
...
\end{enumerate}
In Shopware wird dabei die auf der Detailseite eingestellt Anzahl berücksichtigt. Gleiches gilt implizit für Anforderung SW.F05.

In Kapitel \ref{subsubsection:tcsiteArchitektur} wurde bei der Analyse der Konfiguration in TCsite festgestellt, dass diese erst im letzten Step abgeschlossen werden kann. Äquivalent resultiert für das Shopszenario die Anforderung SW.F06, die eine detailiertere Beschreibung der Anforderung SW.F05 darstellt:
...
\end{enumerate}
Durch das Umkonfigurieren kann es passieren, dass zwei Varianten der gleichen Konfiguration entsprechen. Die Varianten sollen jedoch weiterhin einzeln umkonfigurierbar sein. Daraus resultiert Anforderung SW.F13:
...
Ein eShop bietet nicht nur konfigurierbare Artikel an. Dementsprechend muss das Plugin einen Mechanismus anbieten, über den im Backend eingestellt wird, welche Artikel über das Plugin konfigurierbar sind und welche nicht. Daraus ergibt sich Anforderung SW.F18:
\begin{enumerate}[SW.F18:]\bfseries
\item Das Shop-Plugin muss dem Administrator im Backend eine Angabe darüber ermöglichen, ob ein Artikel über das Plugin konfigurierbar ist.
\end{enumerate}
Wenn ein Artikel über das Plugin konfigurierbar ist, stellt sich die Frage, welchem Konfigurationsmodell es entsprechen soll. Im Fazit des vorangegangenen Kapitels wurde festgelegt, dass die Konfigurationsmodelle nach wie vor von TCsite verwaltet werden. Dort sind Konfigurationsmodellen den Produkten zugeordnet. Daraus ergibt sich Anforderung SW.F19:
\begin{enumerate}[SW.F18:]\bfseries
...
\end{enumerate}
Im Fazit des vorangegangenen Kapitels wurde festgelegt, dass über die Oberfläche die gewählten Optionen des Shopkunden erfasst werden, der technische Konfigurationsprozess aber im TCsite-Plugin stattfindet. Daraus ergibt sich Anforderung TC.04:
