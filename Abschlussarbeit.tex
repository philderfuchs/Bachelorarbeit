...
Wird die Konfiguration über die 'Ok' Schaltfläche beendet, kehrt TCsite zum Quotation Modul zurück. Dort erscheint das konfigurierte Produkt in der Liste der QuotationItems.

\subsubsection{Erweiterbarkeit}

Zur Erweiterbarkeit der Funktionalität bietet TCsite ein Pluginkonzept, welches Abbildung \ref{fig:tcsiteLowLevelExtensions} visualisiert. Neben den oben diskutierten Verantwortlichkeiten definiert die Plattformschicht und die Module sogenannte Extensionpoints \citep{tactonTCsiteApiDocu}. Extensionpoints sind das, was in anderen Erweiterungskonzepten als Events bezeichnet wird: Ereignisse imKontrollfluss der Geschäftsprozesse. Plugins können Methoden bereitstellen, die an den entsprechenden Ereignisstellen aufgerufen werden und so erweiterte Funktionalitäten anbieten \citep{tactonTCsiteDevelopmentManual}.
...
Jedes Plugin hat Zugriff auf das Repository, die Plattform und das Moduls, welchem es eingeordnet ist. Die Plattform stellt außerdem ein paar spezielle Erweiterungspunkte bereit. Sie erlaubt unter anderem die Definition von Webcontrollern. Das bedeutet: Methoden, die auf HTTP-Requests an bestimmte URIs innerhalb der TCsite-Addressraums reagieren.

\textbf{Fazit}\\
In TCsite werden keine Varianten verwaltet, sondern Produkte mit Konfigurationsmodellen, aus denen im Rahmen von Quotations Variantenausprägungen generiert werden. Der Konfigurationsprozess ist im Configuration-Modul gekapselt. In diesem wird eine Nuzerschnittstelle erzeugt, die Verarbeitung der Konfigurationsaufgaben wird allerdings durch den TCserver realisiert. Dadurch ist die Implementierung einer Nutzerschnittstelle in einem anderen System denkbar, während der eigentliche Konfigurationsprozess nach wie vor in der Konfigurationsengine passiert. TCserver ist nicht unmittelbar ansprechbar, jedoch erlaubt das Pluginkonzept die Definition neuer Serviceendpunkte. An diesen können HTTP-Request ausgewertet werden. Das erlaubt die Entwicklung eines Webservices, der eine Produktkonfiguration anbietet. Potentieller Client dieses Services ist jedes System, welches HTTP nutzen kann. Das trifft auf jede Webanwendung zu. eShops-Systeme sind per se Webanwendungen. Somit können Bedingungen an das eShop-System für das Integrationsszenario formuliert werden: (1) es muss modifizierbar sein, (2) die Modifizierung muss einen Webservice in einer Client-Server Architektur nutzen können und (3) die Verwaltung von Varianten muss integraler Bestandteil des Systems sein.
