...
In Kapitel 7 wird die Lösung diskutiert, wobei Potenziale für zukünftige Entwicklungsarbeiten aufgezeigt werden. Abschließen erfolgt eine anwendungsbezogene Einordnung des Resultats in den ökonomischen Kontext, der im Grundlagenteil umrissen wurde.
...
Die Beobachtung der Unternehmensrealität zeichnet ein anderes Bild. Neue Organisationsprinzipien, Informationsverarbeitungspotenziale und Produktstrukturierungsansätze ermöglichen einen Kompromiss aus Preis- und Leistungsführerschaft \citep{schuh05}. Das Ergebnis wird als \emph{hybride Wettbewerbsstrategien} bezeichnet. Eine dieser Strategien ist die sogenannte \emph{\ac{MC}}.
...
Die Definition schließt die Kommunikation heterogener Systeme ein. \glqq Zwischen Systemen\grqq{} differenziert gleichzeitig klar von der klassischen Verwendung eines Programms, bei der ein (menschlicher) Anwender mit einem System kommuniziert. \citet{tilkov11} bemerkt, dass Webservices damit sehr weich definiert sind -- \glqq nämlich eigentlich gar nicht\grqq{}. Fest steht, dass hier ein System einen Dienst als Schnittstelle anbietet, welche von einem Client über Webtechnologien ansprechbar ist. Webservices sind demzufolge eine Möglichkeit zur Realisierung von Integrationsszenarien webbasierter Systeme.
...
Hypermedia beschreibt das Prinzip verknüpfter Ressourcen. Dies ermöglicht es dem Client, neue Ressourcen zu entdecken und bestimmte Prozesse auszulösen \citep{wilde11}.
...
Shopware behauptet in der offiziellen Funktionsübersicht, bereits in der Standardinstallation einen Konfigurator zu besitzen \citep{shopware5Funktionsuebersicht}. Im Folgenden wird dessen Implementierung sowie Integration in den Einkaufsvorgang analysiert.
...
\end{enumerate}
Im Zwischenfazit des Analysekapitels (Kapitel \ref{TCsiteFazit}) wurde festgelegt, dass über die Oberfläche die gewählten Optionen des Shopkunden erfasst und die daraus resultierenden Eingabeparameter an das TCsite-Plugin weitergegeben werden. In diesem wird der KE-Input definiert. Daraus ergibt sich die folgende Anforderung:
\begin{enumerate}[TC.F04:]\bfseries
\item Das TCsite-Plugin definiert die KE-Inputs.
\end{enumerate}
...
\item Das TCsite-Plugin muss den beschreibenden Text einer Variante bereitstellen.
\end{enumerate}
...
\subsection{TCsite-Plugin}
\label{nichtfunktionaleAnforderungenTCsite-Plugin}
...
Aus den Anforderungen geht hervor, dass eine Zuordnung von Shopware-Artikeln zu TCsite-Produkten vorgesehen ist. Somit entsteht abgesehen von Anforderung SW.F18\footnote{Die Angabe im Backend darüber, welchem TCsite-Produkt ein Shopware-Artikel entspricht.} kein zusätzlicher manueller Verwaltungsaufwand im Shopsystem. Es müssen also shopseitig zum Einsatz des Tacton Produktkonfigurators keine weiteren Dateien (z.B. Konfigurationsmodelle) angelegt werden, da der Datenbankbestand von TCsite genutzt wird. Inwiefern aus Anforderung SW.F03\footnote{Die Variantenintegration in den Einkaufsvorgang.} neue Datenbankentitäten in Shopware resultieren, wird im folgenden Konzeptionskapitel geklärt. Davon unabhängig ist die Entscheidung, ob in TCsite extra für den Shopeinsatz angefertigte Konfigurationsmodelle hochgeladen werden oder die bereits vorhandene Produktpalette genutzt wird.
...
\begin{minipage}{\linewidth}
	...
	\captionof{figure}[Ändern und Laden der \emph{configuration}-Ressource]{Ändern und Laden der \emph{configuration}-Ressource}
...
Im Eingang dieses Kapitels wurde erwähnt, dass über die \emph{ordernumber} einer Variante (Tabelle: \emph{s\_articles\_details}) die bestellte Warenkorbposition vom Vertrieb dem Produktkatalog zugeordnet werden kann. Die \emph{ordernumber} wurde im eben dargestellten Ablauf jedoch automatisiert generiert. Allein durch die Bestellbestätigung ist also noch nicht klar, welche Variante hier eigentlich beauftragt wurde. Zur Lösung des Problems gibt es zwei Ansätze:
\begin{enumerate}
...
\emph{Quotations} und \emph{QuotationItems} sind nicht nur Mittel zum Zweck der Erstellung eines \emph{Configurables}. Das Auswerten von Preisinformationen und die Zusammenstellung der Kurzbeschreibung werden ebenfalls durch API-Methoden des \emph{QuotationService} realisiert. Die entsprechenden Funktionen benötigen neben dem KE-Output und dem \emph{Configurable} auch die zugehörige \emph{Quotation} und das \emph{QuotationItem}. Zusammenfassend wird festgestellt, dass jede Verarbeitung eines \emph{Configurables} nur im Kontext einer \emph{Quotation} möglich ist.
...
\vspace{2em}
\begin{enumerate}
\item[\textbf{Bestehende \emph{configuration} löschen:}] Entspricht der HTTP-Methode DELETE.
\begin{addmargin}[25pt]{0pt} 
...
In diesem Kapitel fand ein Entwurf der Plugins statt, wobei die zu verwendenden Erweiterungspunkte der Implementierung bereits vorweg genommen wurden. Während für das TCsite-Plugin nur ein Extensionpoint relevant ist (Webcontroller), stellen sich die logischen, Daten- und Template-Erweiterungen im Shopware-Plugin deutlich kleinteiliger dar. Eine genaue Besprechung der Implementierungsdetails jeder Erweiterung würde über den Rahmen der Arbeit hinausgehen. Demgegenüber ist eine Betrachtung der Herangehensweise an die Umsetzung des vorausgehenden Konzepts angesichts der Menge an Erweiterungspunkten interessant. Dies findet im folgenden Kapitel statt.
...
\subsection*{Zusammenfassung}
Zunächst wurde eine Grundlage für die Kommunikation von TCsite mit externen Systemen geschaffen (CORS/Integrationsuser). Danach wurden die Serviceendpunkte im TCsite-Plugin soweit implementiert, dass der Konfigurationsprozess durch einen externen Client durchgeführt werden konnte. Anschließend wurde die Konfigurationsoberfläche implementiert -- zunächst außerhalb von Shopware, dann auf der Detailseite innherhalb des eShop-Systems. Letztendlich wurden die Varianten in den Einkaufsvorgang integriert.
