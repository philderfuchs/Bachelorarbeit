...
Aus der Anforderungsanalyse ist hervorgegangen, dass zwei Plugins konzipiert werden müssen. Es wird die Vorgehensweise gewählt, zuerst das Shopware-Plugin und danach das TCsite-Plugin zu behandeln. Grund dafür ist, dass das TCsite-Plugin einen Service darstellen wird, den das Shopware-Plugin als Client nutzt. Indem der Client zuerst konzipiert wird, ergeben sich daraus die Daten und Prozesse, die zum funktionieren vom Service bereitgestellt werden müssen. Auf dieser Grundlage kann dann das TCsite-Plugin konzipiert werden. Das bedeutet: welches Webservice-Paradigma wird gewählt, welche Serviceendpunkte werden zur Verfügung gestellt und wie werden diese implementiert.

\subsection{Shopware-Plugin}
Vor der Konzipieren des Shopware-Plugins wird betrachtet, wie sich Artikel, Varianten, Warenkorbpositionen etc. zueinander im Datenbankschema von Shopware verhalten. Daraus wird abgeleitet, wie die per Plugin konfigurierten Varianten in den Einkaufsvorgang integriert werden. Es geht um die Frage: müssen dazu neue Klassen bzw. Tabellen erzeugt oder können bestehende Strukturen verwendet werden? Daraus lässt sich ein neues Aktivitätsdiagramm entwerfen, welches die Grundlage der weiteren Konzeption darstellen wird.
...
\item Werden Warenkorbpositionen umkonfiguriert, entsteht keine neue Warenkorbposition, sondern die Änderung passiert auf der Warenkorbposition selbst.
\end{enumerate}
...
Stattdessen startet nun der Konfigurationsprozess über das Shopware-Plugin. Dazu müssen erstmalig Daten vom TCsite-Plugin geladen werden. Diese Daten werden im Rest der Konzeption des Shopware-Plugins unter dem Begriff \textbf{configuration} zusammengefasst. \textbf{configuration} ist aus Shopware-Sicht eine Ressource, welches alle notwendigen Informationen beinhaltet, um eine Variante zu bilden. Das ist nun deutlich abstrakter als beim alten Mechanismus zur Bildung von Varianten in Shopware, wie er in Kapitel \ref{subsubsection:shopwareKonfiguration} vorgestellt wurde. Bei diesem Waren die notwendigen Informationen \emph{Gruppen} und \emph{Optionen}.

Gemäß Abbildung \ref{fig:konzeptionEinkausvorgang1} fällt nach Aufruf der Detailseite die Entscheidung, ob es sich um eine neue Konfiguration oder eine Umkonfiguration handelt. Letzteres bedeutet: Es wird eine Variante angezeigt, die bereits mit Hilfe der \textbf{configuration} gebildet wurde. Wie im Falle des \emph{korrespondierenden TCsite-Modells} muss hinterlegt werden, ob bereits eine \textbf{configuration} besteht und wenn ja, wie sie identifiziert wird. Dementsprechend wird in der Attributtabelle \emph{s\_articles\_attributes} eine Spalte \emph{configurationID} angelegt. Sie beinhaltet den Identifikator der Ressource beim Webservice.
