...
\section{Fazit}
\label{section:Fazit}
Im Folgenden findet eine Diskussion der Ergebnisse statt. Dies geschieht sowohl aus einem technischen, als auch einem anwendungsbezogenen Standpunkt. Dabei werden Alternativen und Potentiale für die zukünftige Entwicklung aufgezeigt.

Hier erstmal kicken, dass alles geklappt hat und dass Shopware nur exemplarisch verwendet wurde

Es wird mit der Bewertung des Webservices begonnen. In der Konzeption des TCsite-Plugins wurde die Behauptung aufgestellt, dass dieser das REST-Architekturmuster erfüllt. Im Grundlagenteil der Arbeit (Kapitel \ref{subsubsection:REST}) wurde erwähnt, dass zur abgestuften Beurteilung der REST-Konformität das \glqq Richardson Maturity Model\grqq{} existiert \citep{wilde11}. Es Unterscheidet vier aufeinander aufbauende Stufen, je nachdem, welche Kriterien erfüllt werden:
\begin{enumerate}
\item[\textbf{Level 0:}] Dokumentenaustausch via HTTP. Auch SOAP würde dieses Kriterium erfüllen.
\item[\textbf{Level 1:}] Exponierung addressierbarer Ressourcen. Die Interaktion mit dem Webservice findet dabei nur über diese Ressourcen statt.
...
\begin{enumerate}
\item[\textbf{Level 0:}] Das Plugin verwendet Webcontroller, welche HTTP-Requests entgegennehmen und beantworten. Damit ist die Bedingung für Level 0 erfüllt.
...
\item[\textbf{Level 2:}] In Kapitel \ref{subsubsection:REST} wurde erwähnt, dass laut \citet{richardson07} die uniforme Schnittstelle verletzt wird, wenn \emph{POST} im weiteren Sinne, d.h. zum Starten von Prozessen, verwendet wird. Damit wird Level 2 nicht erfüllt. Da die Stufen aufeinander aufbauen, wird Level 3 nicht mehr geprüft.
\end{enumerate}

Der Implementierung des REST-Webservice entspricht also Level 1 des \glqq Richardson Maturity Models\grqq{}. Die Implementierung ist an der Einhaltung der uniformen Schnittstelle gescheitert. Verarbeitungsschritte sind aber inhärenter Bestandteil des Konfigurationsprozesses. Damit wird klar, dass sich eine Produktkonfiguration nicht problemlos auf einen REST-Webservice übertragen lässt. Das Erweiterungskonzept von TCsite hat durch die Bereitstellung von Webcontroller den REST-Ansatz für eine prototypische Entwicklung leicht zugänglich gemacht. Es ist zu überprüfen, ob eine Konfigurationsprozess bei Integrationsszenarien nicht besser über die RPCs des SOAP-Ansatzer bewältigbar ist.

Sicherheitsaspekte müssen aus dem Buch entnommen werden...

In der momentan entworfenen Lösung wird jedes mal, wenn die Detailseite eines konfigurierbaren Artikels in Shopware aufgerufen wird, eine Quotation in TCsite angelegt. Zu keinem Zeitpunkt wird überprüft, welche der Quotations mit einer Variante im Zusammenhang steht, die tatsächlich bestellt wurde. Wird eine Detailseite nur aus Interesse geöffnet, die Konfiguration abgebrochen oder der Warenkorb nie in eine Bestellung verwandelt, werden \glqq \emph{Quotation}-Leichen\grqq{} in der Datenbank von TCsite akkumuliert. Damit wirkt sich der Entwurf negativ auf die Performance von TCsite aus, was nicht im Sinne eines Integrationskonzepts sein kann. Allerdings wurde in Kapitel \ref{subsection:TCsite-Plugin} begründet, dass das Erstellen von \emph{Quotations} nicht vermieden werden kann. Ein Lösungsansatz ist die Entwicklung eines Garbe-Collection-Mechanismus, der unnötige \emph{Quotations} post hoc entfernt. Außerdem ist zur Reduktion des angelegten \emph{Quotations} zu überlegen, ob ein anderes Verhältnis aus \emph{Configurables} respektive \emph{QuotationItems} zu \emph{Quotations} wird. Intuitiver als die bisherige Lösung wäre die Registrierung einer \emph{Quotation} pro Warenkorb in Shopware. Für jede Warenkorbposition gäbe es dann ein \emph{QuotationItem} in einer \emph{Quotation}. Weiterhin kann der Lebenszyklus von \emph{Quotations}, der in Kapitel \ref{subsubsection:tcsiteArchitektur} angesprochen wurde, genutzt werden, um neben der Container-Funktionalität eine \emph{Quotation} auch dessen Ausdruckskraft zu nutzen. Die Einrichtung zusätzlicher Zustands, wie z.B. \glqq Bestellung im Shop ausgelöst\grqq würde Vertriebsmitarbeitern die Verwaltung der Bestellungen erleichtern.

Während der Konzeption wurde das Problem angesprochen, dass der Vertrieb die bestellten Varianten interpretieren können muss. Der momentane Lösungsentwurf ist das Öffnen der Varianten in TCsite. Das erfordert zur Behandlung einer Bestellung das Wechseln zwischen zwei Anwendungen -- Shopware und TCsite. Neben dem umständlichen Workflow setzt diese Lösung implizit voraus, dass der entsprechende Mitarbeiter überhaupt Zugang zu TCsite hat. Da TCsite ein kostenpflichtiges Lizenzmodell hat, ist das nicht nur eine technische Problematik. Ein Lösungsansatz wäre das Bereitstellen weiterer Dokumente in Bestellungsbereich von Shopware, wie z.B. Stücklisten oder Konstruktionszeichnungen.

Momentan gewährleistet die Konfigurationsoberfläche nur, dass der Shopkunde eine korrekte Variante bestellt (vorausgesetzt, das Konfigurationsmodell ist richtig definiert). Ansonsten hat der Konfigurationsprozess für den Anwender keinerlei informativen Charakter. Abgesehen von der Feldbeschreibung bietet die die Oberfläche keinerlei Begleitinformation. Diese muss noch mit Bildern, beschreibenden Texten oder sonstigen medialen- und interaktiven Elementen angereichert werden. Im Augenblick ist die Nutzung des Konfigurators nur unter der Vorraussetzung möglich, dass der Anwender bereits Kenntnis über alle wählbaren Optionen hat. Davon kann nicht ausgegangen werden, weshalb der momentan Prototyp für den Einsatz im Endverbrauchermarkt noch ungeeignet ist.

Abschließend wird das Ergebnis der Entwicklung in den ökonomischen Bezugsrahmen eingeordnet werden, der zu Beginn der Arbeit eröffnet wurde. Das Potential der Integration wird sowohl im B2B- als auch B2C-Bereich erläutert.

Im B2B-Bereich ist der entscheidende Faktor der Implementierung, dass Artikel, die im Shop konfiguriert wurden, in TCsite geöffnet werden können. Damit wird dem CPQ-Prozess eine neue Dimension gegeben. Mitarbeiter des Kundenunternehmens können Artikel selbst konfigurieren, welche daraufhin die Basis zur Angebotserstellung im anbietenden Unternehmen darstellen. So können die einzelnen CPQ-Bestandteile noch stärker auf die jeweiligen Verantwortungsbereichen aufgeteilt werden -- der Kunde drückt seine Vorstellungen in der Konfiguration aus (C..), der Anbieter führt die Preiskalkulation und Angebotserstellung (..PQ) durch.

Aus B2C-Bereich wurde durch die Arbeit ein Mass Customization Tookit umgesetzt. Durch die Verwendung von Open-Source-Technologie berücksichtigt die Implementierung die Grundwerte des \ac{MC}, die sich in der Individualisierung bei gleichzeitiger Kostenreduktion ausdrücken. Die Verwendung frei zugänglicher Software ist ein eindeutiges Einsparpotential. Somit wird eine Weiterverfolgung der Entwicklung nicht nur von technischem, sondern auch wirtschaftlichem Interesse sein.

;;;;;;;;;;;;;;;;;;;;;; Nun Bezug auf den Einleitungsgang, falls es gelingt... erwähnen, dass Konfigurationsmodelle von Tacton sind nicht auf eine bestimmte Domäne beschränkt sind...
